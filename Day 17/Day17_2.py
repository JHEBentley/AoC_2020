            ...
            for k in [-1, 0, 1]:
                for l in [-1, 0, 1]:
                    ...
                    if i == 0 and j == 0 and k == 0 and l == 0:
                        continue
                    if 0 <= x + i < cubes.shape[0] and 0 <= y + j < cubes.shape[1] and 0 <= z + k < cubes.shape[2] and 0 <= w + l < cubes.shape[3]:
                        if cubes[x + i][y + j][z + k][w + l] == "#":
                            activeCounter += 1
...
cubes = cubes[:, :, numpy.newaxis, numpy.newaxis]
...
while cycles < numOfCycles:
    ...
    cubes = numpy.pad(cubes, ((1, 1), (1, 1), (1, 1), (1, 1)), constant_values=".")
        ...
        for y in range(cubes.shape[1]):
            for z in range(cubes.shape[2]):
                for w in range(cubes.shape[3]):
                    ...
                    oldCubes = cubes[x][y][z][w]
                    newCubes[x][y][z][w] = GetNewCubes(activeCounter, oldCubes)
...
print(sum([1 for x in cubes for y in x for z in y for w in z if w == "#"]))